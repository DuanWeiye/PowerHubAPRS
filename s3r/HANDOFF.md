# S3R 交接文档（2026-09-12 更新：v0.3.1 进楼飞点修复上机，供新对话续做）
...
- **2026-09-12 深夜：v0.3.1 已 OTA 上机（进楼飞点修复，见下方「0912」段）**：nav_core 加
  "断档后重捕获可达性门"+车辆证据放行+重建速度初始化+0,0 坏句拒收；单测 21 项、四份数据回放、
  台面 --reacq 注入全过。PowerHub 固件未动（fallback 管线同源漏洞仍在，见待办）。
- **2026-08-22 晚：P3 上机完成（S3R v0.3.0 + PowerHub 同步更新，见下方「P3」段）**：
  S3R 以 nav_core v2 改写 GGA/RMC，PowerHub 收 $PFUSE 心跳旁路自家野点门/KF，上传报文带
  融合遥测（nav/sig/still/cn0/rej，服务端已加列）。台面注入测试（不出门）全链路通过。
  设备当前状态：S3R app1 v0.3.0、fuse ON（NVS）、PowerHub 已刷，**可直接外场 A/B（P4）**。
- `s3r/` 固件 v0.1.1 曾刷入真 AtomS3R 并运行：GPS↔链路双向透传、NMEA 旁路解析、
...
## ★2026-08-22 第二份外场数据（雨天、山手线+博物馆）：验证点 1/2 通过 + 日志重复写 bug

当天行程（主人确认）：田町 →山手线→ 上野，国立博物馆（全程室内，蜂窝也不通），→山手线→
五反田，地下餐厅（有蜂窝无定位），→山手线→ 田町。10:34 开机 → ~17:30 到家，7.9h，
**有定位仅 17%**（好定位 4309s）。无定位大段对得上行程：11:28–14:30 博物馆（12:04–12:24、
13:04–13:14 的零星定位=馆区楼间室外走动）、15:25–17:01 地下餐厅——PowerHub 每分钟 GSV 均为
可见星 9–13 颗但 CN0=0（没天空）。**山手线三段（地面线）定位逐分钟**：
- 田町→上野 10:53–11:08：出门后 14min 才首次定位（10:44 出门 CN0 峰 30–43 仍无 fix，6 天未
  开机，疑冷/暖启动在边缘信号下下载星历慢），10:58 起全程 fix，但 sats 4–7、hdop 1.6–2.1
  （偶 16–18）；
- 上野→五反田 14:51–15:15：上野→有乐町 fix ~100%（sats 6–11、CN0 峰 23–31），**有乐町→
  五反田 15:01–15:14 零 fix，CN0 峰 0–15 dBHz、可见星 13**（接收机什么都跟不住）；
- 五反田→田町 17:11–17:20：fix 11–56%、sats 4–6、hdop 3–9，17:15–17:19 零 fix，CN0 峰 16–24。
对比 0808 电车段：valid 74%、sats 中位 15、hdop<2 占 90%——今天车内接收明显差于 0808（同一
接收机，0822 步行时 24 星 CN0 45 正常）。S3R 纯透传、GSV 直接来自接收机，**不是固件问题**；
0801 已量化 CN0max≲27dBHz 保不住 fix，今天车内峰值 0–24 远在其下。可疑变量（待控制实验）：
雨天湿包/衣物贴着无源贴片天线（水吸收 L 波段）、天线朝向（贴片有方向性）、有乐町–品川段
东侧紧贴新干线高架。全速率层只覆盖 17:27 起（到家后），当天分析全靠摘要层 1Hz。
电量 98mA / 22.4h，同 0816。

### 验证点结果
1. ✔ **电车巡航可分性**：有定位的电车巡航 288s，设备 acc_std_max p50 0.20 / p90 0.37 /
   max<1.0，**0% 超 1.0**；步行 p50 2.53、87% 超 1.0；在家静置 0.01。博物馆/车内无定位段
   （IMU 单独看）近似 carried 触发率 1.5–19%（走动/晃动的短促脉冲，仅让 coast σ 长得更快，
   保守方向）。**accStdActive=1.0 定版。**
2. ✔ **E=0**（v2 与基线）。A 7.5/14.7 vs 基线 7.3/13.2（1Hz 回放 v2 略松，与 0816 结论
   一致，非退化）；B 0.8 vs 1.0；C 4 vs 3——v2 多的 14:54:32 那个 68m 是 KF 对原始 150m
   断崖的两步收敛（基线一步跨过，dt≥10s 不计），地图上等价；D 桥接 95 拍 / 最长 19s /
   σmax 39.8 / D2 14.3m，**小时级长断档零幻影桥**（coast 15–19s 即诚实停发）。
3. △ **真实停留**：5 段开阔天空停留（hdop 0.7–1.0），raw r95 本就 1–2m，v2 持平基线。
   但 **still 信念建立慢**：4 段新停留里 3 段花 55–98s 才过 zuptOn（标称 stillEnterSec=12）
   ——1Hz 下 KF 速度噪声经常超 stillSpdMax=0.7 → 信念来回泄放。开阔天空下无害（raw 已
   够紧），真正需要 ZUPT 的弱信号停留里创新门 8m 也会卡。**待 5Hz 全速率停留段 / P3 上机
   遥测再判，本次不动参数。**

### 新发现
- **ATGM336H 冻结保持解再现**（0816 进楼 191m 同病灶）：14:50:32–14:51:01 上野站发车，
  接收机 hdop 9.7 坐标不动、报速 0.1–0.6，重捕后一口气跳 280m（基线同样接受，C 指标
  两版 1:1）。"零位移∧报速>1" 签名统计：0822 19 拍 / 0816 69 / 0808 105，绝大多数是
  单拍；致害那次报速 0.1 根本不触发 → **不加门**，记为 raw 残留。
- **首次定位慢**：6 天没开机后 10:44 出门 CN0 已 38–43，却 10:52 才首次定位、11:04 才好
  定位（暖启动下载星历 + 雨天街道），观察项，暂不追。
- **★日志"重复写"bug（解码器已修，固件待修）**：bin 里出现"上一段数据写了两遍、第一遍在
  半条记录处截断"的结构——常态运行 6 次整 2.0–2.9s 的块（17:36–18:08，间隔不规则）；
  拉取时**正在写的活动段**另有 65s/30s 级的陈旧内容（IMUDUMP 用第二个句柄读一个还开着
  写句柄的文件，LittleFS 不支持同文件双开）。老解码器"未知字节跳 1B"在这儿失步，把
  14:59–15:00 整段读成垃圾（表现为 74s 空洞 + LATCH_ON 垃圾事件）。
  **imu_decode.py 已改**：记录须与后继 tms 连贯才接受，失步时 64B 窗内搜 ≥4 条连贯记录
  重对齐，(字节相同 ∧ 120s 窗) 去重 → 0822 坏字节 694→166、去重 552 条、空洞归零；
  0816 回归也去掉 1058 条重复（当时的"0.01% 损坏行"就是它）。
  **固件两处待修（下次 OTA 一并）**：① IMUDUMP 前 `r.f.close()`，dump 后以 "a" 重开
  （IlRing 需存当前文件名）；② ilFlushBuf 检查 write()/flush() 返回值，失败计数 +
  LittleFS 剩余 KB 进 $S3R,INFO 与 EVT——常态 2s 重复的根因（疑 NOSPC/CoW 失败重试）
  要靠这个定。
- 工具：当天分析脚本（按 TIMEMARK 对齐 JST、按 ms 回退切开机段、按原始速度分
  电车/步行/停留/在家 看 _feat 分布、10 分钟剖面）在会话 scratchpad day.py，思路可照抄。

## ★2026-08-22 晚：P3 上机完成（S3R v0.3.0 / PowerHub 同步）——读这段再改融合链

### 设计（已实现）
- **S3R fuse.ino 整体重写为 v2**：v1 三开关（锁存/DR/逃生门）全部删除；唯一估计器 =
  `nav_core.h`（P2 定版参数，三份数据验证）。每拍语句序实测 **RMC→VTG→GGA**：RMC 先暂存
  （≤300ms 兜底放行），GGA 到达驱动估计器（同拍 hdop + 同拍 RMC 原始速度，与回放输入语义
  完全一致），然后按同一份输出先放改写后的 RMC 再放 GGA。IMU 按 0.25s 块喂，accStd 取
  1s 滑动最大（= 摘要层 FEAT.accStdMax = 回放 feat=device 语义）。
- **输出规则（诚实优先）**：有点→坐标换估计，推算点 GGA q=6 / RMC 模式 E；无点且原句有定位
  （被野点门拒）→ GGA q=0 / RMC V（下游不采信）；原句无定位→原样；fuse 关/坏行/其它句→透明。
  改写函数全在 `nmea_rw.h`（nmeaGgaApply/Invalidate、nmeaRmcApply/Invalidate），主机单测
  55 项全过（test/nmea_rw_test.c，含真机语句 + S/W 半球）。
- **$PFUSE v2（1Hz）**：`$PFUSE,2,<en>,<mode P/F/C/N>,<σm>,<still>,<spd>,<hdopEma>,<rej>,
  <carried>,<accStd1s>,<headRate>*cs`。PowerHub：en=1 且 40s 内有心跳 → `s3rNavActive` →
  `s3rFeedLiveFix` 直接进 liveFix（不过野点门/KF，est 由 GGA quality=6 标出）；心跳消失/en=0
  → 回落 `gnssFeedLiveFix` 并 `gnssPipelineReset()`。保持期 40s 是因为发包阻塞 15–25s 会把
  心跳和 NMEA 一起丢掉（5s 曾让每次 beacon 后离线/在线翻一次白重置 KF）。
- **上传遥测**：TrackPoint v2（25B）加 nav(b0 S3R 在线 b1 推算点 b2 自家 KF)/sig(σm)/still
  (0-100)/cn0/rej(本周期拒绝数差分)；fmtPoint 多 5 个键；flashlog 段布局 `/t/fmt` 版本号
  不匹配即清旧段。服务端 iotService `aprs_track` ALTER 加 5 列（老固件存 NULL），地图弹窗
  显示"估计器/信号"行，tests 加 round-trip 用例。
- **控制台新命令**：PowerHub `s3rnav`（心跳解析+liveFix）、`pcon`/`pcoff`（PORT.C 5V 手控，
  接 S3R USB 前先 pcoff）、`gpsin <行>`（原样写到链路：PCAS 配置 / 台面注入）；S3R 协议
  `$S3R,NMEA,<句>`（当 GPS 输出喂估计器，开 3s 注入窗屏蔽真 GPS 定位句）。
- **日志 bug 两处修复**：IMUDUMP 前关活动段写句柄、之后 "a" 重开（双开读陈旧块）；
  ilFlushBuf 记 write() 短写次数 → `$S3R,INFO` 新增 `ilfail=`/`free=`K + EVT 9(IL_WFAIL)。
  INFO 行 ~200 字符，PowerHub nmeaLine 已扩到 256。

### 台面验收（test/inject_test.py，不出门）
`gpsin $S3R,NMEA,…` 注入合成 5Hz 轨迹（测试区 35.05/139.05）：静止 40s → 步行 1.4m/s 60s
→ 断档 20s → 续走 30s → 静止 25s，PowerHub 侧 `s3rnav` 逐段读状态：
- 静止：still 40s 内到 1.00，liveFix 速度 0、坐标钉住；步行：mode F、σ 1.2–1.4m、KF 速度
  1.2–1.4m/s；断档 10s：mode C、est=1（lastGGAq=6）、σ 22.7m 诚实增长；恢复：σ 回 1.2m、
  终点与理论位置差 3m、无传送；旁路全程 ONLINE。
- 上传：beacon 正常触发（first/moved），服务端收到 `nav=1/3,sig,still=100,cn0` 遥测点
  （测试点已删）。`s3rfuse off` → PowerHub 立即回落自家 KF、注入定位原样透传；`on` → 旁路。
- 注入结束后真 GPS（屋内无定位）接管：still=1 的"停留保持"推算 ≤120s 后停发，符合设计。
- 已知台面现象：PowerHub 发包阻塞期间注入流被 RX 溢出截断（`$S3R,ERR,unknown` 1 行）——
  测试夹具现象，外场 GPS 流进 S3R 不经 PowerHub，不受影响。

## ★2026-09-12 首次 P4 外场（S3R v0.3.0 全天 fuse ON）→ 进楼飞点根因 + v0.3.1 修复

数据：`logs/pwrlog_20260912_221946.*`、`s3r/imulog_20260912_221953.bin`（+_today_sgnss/_feat 切片，
gitignored）、`logs/track_20260912_*.png`（原始 vs 融合、OSM 水域对照）。环形已清空。

### 结果
- **电量** 13.7h 平均 105mA（100→21%，折算 20.9h）；点数 569 为历次最多。同配置历次 92–100mA，正常。
- **步行段（18:22–18:47 沿运河北行、19:15–19:31 回家）**：原始 1Hz 星 14–20/hdop 0.7–1.0，
  21s 滑均残差 p90 3.5m；服务端点与同秒原始差 p50 1m/p90 9m；OSM 水域多边形核对：42 点中 2 点
  在桥上碰水（≤24m）。**融合既没改善也没恶化**，主人看到"飞到河里"的是下面这个点。
- **★到家飞点 19:32:49（103km/h，σ19，落在家附近的河道里）根因**：19:31:43 进楼最后一拍
  (sats6/hdop4.4) → 63s 无定位 → 19:32:47 接收机单拍吐出 424m 外伪定位(sats6/hdop3.8/自报 1.9m/s)
  → nav_core `glResyncMs=30s`"参照超龄→放行重建"**无任何门**，navReset 到伪点 → 4s 后又一拍差
  97m，initVelVar=2500 让 KF 速度冲到 24m/s → 输出 103km/h。**PowerHub track.ino GLITCH_RESYNC
  同源逻辑有同样漏洞（仅在 S3R 心跳丢失回落时生效）。**
- 其它：17:53:46 电车进站减速+20s 断档 coast 超前 198m（匀速外推对减速车辆固有弱点，hdopEma 没到
  2.5 门，未处理）；19:31 发包阻塞 >40s → 心跳丢 → 回落自家 KF 一拍（nav=4，40km/h 伪速度）；
  SGNS 有 4 条 valid=1 但 lat/lon=0,0（sats 0–3）；摘要环里仍有 08-22 台面注入数据，
  **多开机段分析前必须按 ms 回退切片**（本次 `imulog_20260912_today_*.csv` 即切片）。

### v0.3.1 修复（nav_core.h，全部经 test/nav_core_test.c 场景单测 + replay 四份数据回归 + 台面）
1. **断档后重捕获可达性门**（`reacq*` 参数）：断档前是行人语境（上次接受量测后 KF 速度
   `fixSpd` < 3m/s）且参照年龄在 5s–180s 内 → 新定位到参照的距离必须 ≤ 年龄×4m/s + 30m，
   否则拒收（不重建、不更新参照 → 界限随时间增长，人真走到了迟早放行）；>180s 门关闭
   （进店/上车后一切皆有可能）。车辆语境（断档前速度高）完全不受影响。
2. **车辆证据放行**（`reacqVehEvidenceMs=2000`）：被门拒收的候选若连续自报速度 ≥3m/s 达 2s
   → 放行并**按重建处理**（KF 状态已不可信，不能用正常更新去"拖"）。0808/0912 回放证实：
   电车出站/进站前后重捕获自报 14–23m/s，不放行会被门压 40–170s（今天 08:47 就是 66s）；
   加放行后只多等 2–3s。伪定位自报步行量级（0912 为 0.9–1.9）→ 不放行。
3. 门拒收期间 **coast 不发点**（歧义窗，同野点门连拒窗语义）：否则 0808 回放里等待 2s 续发的
   coast 把桥接终点误差从 20m 推到 290m。
4. 重建时速度从 RMC 自报速度/航向起算（`initVelVarRaw=100`），不再 σ50m/s 起步。
5. 0,0 坐标一律不是定位（nav_core 与 fuse.ino 双保险）。

回放回归（新 vs 旧）：0816/0822 逐拍差异 ≤70 拍且指标同级；今天 19:32 四拍伪定位全拒、无输出；
0808 电车段：A p90 15.5→14.6 改善，D2 终点修正 19.9→292m **是指标暴露而非退化**（旧版靠 KF
拖影把"停留保持 coast 钉住而电车已开走 235m"的错桥抹平成 2m；新版拒 2.4s 后诚实重建）。
台面 `inject_test.py --reacq`（0912 复现 + 电车出站）：D/F 伪定位 rej 计数涨、liveFix 不动；
H 原地真定位立即接受；J 电车 2s 后放行、liveFix 跟上 20.9m/s。测试点已从服务端删除。

### 待办追加
- PowerHub fallback 管线（track.ino 野点门/KF）与 nav_core 同源但**没有这个门**：心跳丢失
  回落期间同样会盲收。根治=fallback 直接复用 nav_core.h（单一实现），而不是再抄一份门。
- 17:53 类"减速车辆断档 coast 超前"：可考虑用陀螺/加计减速证据或 hdop 上升趋势提前停发。
- inject_test.py 已加控制台在线检查（OTA/imu_fetch 用过桥后 60s 内桥没退出会吞命令）。
...
### P2 数据积累+离线调参 —— 设备侧已完成（0808 深夜），剩外场采数+调参：
  ...
  ✔ 剩余验证（0822 第二份数据，见上方新段）：电车巡航可分性 ✔（0% 超 1.0）、
    新一天 E=0 ✔；真实停留段 △（still 建立慢 55–98s，1Hz 回放现象，待 5Hz/上机遥测再判）。
  ☐ 固件：IMUDUMP 活动段双开修复 + 写失败计数/剩余空间遥测（见 0822 段）。
### P3 上机 —— ✔ 2026-08-22 晚完成（见上方 P3 段），原清单：
...
### P4 外场 A/B 验收（**下一步**）：同路线 s3rfuse on/off；标准=停留成点、断档≤15s 桥接、
  良好段无偏移、飞点为零、全段类不输基线 → 定最终版；调不出净收益则 S3R 永久退守
  透传+看门狗+记录仪。验收数据源：①服务端 aprs_track 的 nav/sig/still/rej 列（实时）；
  ②S3R 环形日志回放（metrics.py）；③PowerHub 电量日志 CN0。
  外场前检查：`s3rinfo` 应见 ver=0.3.0,fuse=1；`s3rnav` 应 ONLINE；山手线车内定位差的
  问题（0822 段）与融合无关，可同机做天线朝向/位置对照。
...
cd test && python3 inject_test.py [--send]   # 台面注入端到端验证（不出门；--send 末尾强发一包，记得删测试点）
python3 imu_decode.py imulog_*.bin   # → _imu/_gnss/_fuse/_events 四个 CSV
cd test && g++ -O1 -Wall -o nav_core_test nav_core_test.c && ./nav_core_test   # nav_core 场景单测
cd test && python3 inject_test.py --reacq   # 台面：断档重捕获（进楼飞点复现 + 电车出站放行）
# S3R USB 控制台：help/info/ping/imu/fuse [on|off]/imulog/imuclear/screen/linkswap/pinscan/reboot
# PowerHub 控制台：s3rbridge / s3rping / s3rinfo / s3rimu / s3rnav / s3rfuse on|off / pcon / pcoff / gpsin <行>
```
